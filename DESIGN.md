...
1. The check-in uses up to ten eligible words. Below-frontier audit slots prioritize untested or due words; remaining slots use due frontier words, prioritizing recent misses. If room remains, supplemental below-frontier audit words fill the check-in so normal sessions do not feel too small. If a learner has no lower-tier or due words yet, eligible frontier words fill the warm-up instead of producing a blank check-in. Everyday learners audit Foundation; Expanding 1 learners audit Foundation and Everyday; Expanding 2 learners audit Everyday and Expanding 1 daily and Foundation every other day. Lower-tier weakness increases that tier's audit slots.
2. The new-word stage chooses unseen words from the learner's tentative frontier. Each profile has a switchable **New words** setting. **Mixed** uses the original weighted frontier sampling, underweighting transparent cognates without banning them. **Topic groups** introduces words in curriculum order by chapter and source category, preferring thematic packets while continuing into nearby packets as needed to reach the normal daily new-word target. For Year 1 chapters whose source category is only a placeholder like `chapter-0`, topic grouping falls back to broad semantic tags such as time, place, number, communication, and education so chapter-level packets do not become vocabulary soup. Both modes gently reduce the normal fifteen-word intake by one word for each eight due-review backlog items, with an eight-word floor while vocabulary remains available. If the current frontier tier has too few unseen words left, the planner finishes those words and then borrows from the next higher tier rather than creating a tiny session.
3. The review stage includes same-day repair words, a capped slice of ordinary due backlog, and that day's newly presented words, split into rounds of at most ten. Same-day repairs and new words are protected; excess ordinary backlog rolls forward instead of making one session feel endless. Successfully audited below-frontier words do not enter ordinary reviews.
...
1. **Check-in:** up to ten eligible words with brief per-answer feedback and automatic advancement. Structured slots audit due or untested below-frontier vocabulary; remaining slots use due frontier words, prioritizing misses, then supplemental below-frontier audit or frontier warm-up words when needed to keep the session substantive. No word may appear in more than one check-in on a calendar day. The first completed quiz on a local calendar date supplies that day's baseline error rate and streak credit.
...
3. **Due reviews:** same-day repairs, newly introduced words, and up to a capped slice of ordinary due backlog, presented in ten-question rounds. Reviews influence new-word intake when a backlog develops, but one full session is not required to clear every overdue word.

The implemented target is fifteen new words per study day when reviews are light, minus one new word for each eight due-review backlog items, with an eight-word floor while enough unseen words remain. Step 3 normally caps planned review work at twenty words, including that day's new words, while allowing same-day repairs to remain protected. After any review round with more planned review words remaining, the learner can either continue today's reviews or end the session and see results; unfinished ordinary backlog stays due. Mixed new-word sampling underweights helpful transparent cognates rather than banning them: non-transparent words use weight `1.0`, moderate cognates `0.6`, and strong cognates `0.35`. Topic-grouped intake preserves source order within chapter/category or chapter/semantic packets so learners do not meet isolated weekdays, numbers, or city words one at a time, but it keeps collecting nearby packets until the normal target is met when vocabulary is available. A future release may also apply the proposed soft cap of approximately ninety first-pass prompts or fifteen minutes. The exact limits remain configuration rather than domain invariants and should be adjusted after observing the learners.
...
- A daily session contains an up-to-ten-word due/audit check-in, topic-aware or mixed adaptive explicit new-word presentations, and capped due frontier/repair reviews in rounds of at most ten.
...
- Newly presented words are reviewed later in the same session; the new-word count is reduced by one for each eight due-review backlog items and normally keeps an eight-word floor, while extra ordinary review backlog rolls forward.
